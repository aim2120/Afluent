...
extension AsyncSequence {
    ...
    ///
    /// - Parameters:
    ///   - receiveExecute: A closure that is invoked when the unit of work is executed. The closure can throw errors.
    ...
    public func handleEvents(@_inheritActorContext @_implicitSelfCapture receiveExecute: (() async throws -> Void)? = nil, @_inheritActorContext @_implicitSelfCapture receiveOutput: ((Element) async throws -> Void)? = nil, @_inheritActorContext @_implicitSelfCapture receiveError: ((Error) async throws -> Void)? = nil, @_inheritActorContext @_implicitSelfCapture receiveComplete: (() async throws -> Void)? = nil, @_inheritActorContext @_implicitSelfCapture receiveCancel: (() async throws -> Void)? = nil) -> AsyncSequences.HandleEvents<Self> {
        AsyncSequences.HandleEvents(upstream: self, receiveExecute: receiveExecute,  receiveOutput: receiveOutput, receiveError: receiveError, receiveComplete: receiveComplete, receiveCancel: receiveCancel)
    }
}
